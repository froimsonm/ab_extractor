...
bool QCompressor::gzipDecompress(QDataStream &input, int inputSize, QDataStream &output)
{
    // Is there something to do?
    if(inputSize > 0)
    {
        ...
        do {
            ...
            input_data = new char[chunk_size];

            // read data from input stream
            input.readRawData(input_data, chunk_size);
            ...
            // Update interval variables
            input_data += chunk_size; //todo: ??? what
            input_data_left -= chunk_size;
            ...
            do {
                ...
                // Cumulate result
                if(have > 0)
                    output.writeRawData((char*)out, have);
